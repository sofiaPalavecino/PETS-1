  ...
  async ngOnInit() {
    ...
    if (this.tipo == "paseador") {
      ...
      if (this.tipo == "cuidador") {
        ...
        this.afs.firestore
          .doc(`cuidador/${this.uid}`)
          .get()
          .then((data) => (this.precioDia = data.data()["precio_dia"]));
      }
    }
    ...
    this.userServ.mascotas.subscribe((data) => {
      data.forEach((element) => {
        ...
        this.mascotasCheck[this.mascotasCheck.length - 1].dataExtra =
          element.docId;
      });
    });
  ...
  checkDisponibilidad() {
    if (this.tipo == "paseador") {
      ...
      this.disponibilidades.subscribe((data) => {
        ...
        if (
          !this.semana[2] ||
          data[0].Miercoles - cantMascotas < 0 ||
          (this.cantidadDias == cantDias &&
    ...
    if (cantMascotas > 0) {
      ...
      if (this.tipo == "paseador") {
        if (cantDias > 0) {
          ...
          nuevoContrato.then((data) => {
            ...
            this.afs.doc(`paseador/${this.uid}`).update({
              solicitud_paseo: firebase.firestore.FieldValue.arrayUnion(
                data.id
              ),
            });
          });
        ...
        if (this.fecha != "") {
          ...
          this.presentAlert(
            "Datos Incompletos",
            "Debes seleccionar el día de inicio del cuidado"
          );
        }
      }
    } else {
      this.presentAlert(
        "Datos Incompletos",
        "Debes seleccionar al menos una de tus mascotas"
      );
    }
  }
